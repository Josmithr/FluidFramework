...
This module has gotten quite large.
We should split it into smaller modules along semantically meaningful boundaries.
We should also unit test these boundaries.
...
### Configurable API report generation

Currently, we offer a couple of configuration options with regards to API report generation.
Longer term, we should make it easier for consumers to fully customize the presentation of their reports, using our API graph representation as input.

I see a few layers of capabilities we can expose for this:
1. An API for opinionated API report generation (like what we currently have, possibly with a couple of different baseline "flavors").
2. Something template / callback based - we provide hooks for users to customize how the API graph is transformed into a report file (we likely handle the I/O in this scenario)
3. We expose the API graph generated by API analysis - the user can then do whatever they want with that.
