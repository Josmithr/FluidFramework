...
describe("Buffer isomorphism", () => {
	...
	it("utf8 base64 conversion is compatible", () => {
		...
		for (let index = 0; index < testArrayUtf8.length; index++) {
			const nodeBuffer1 = BufferNode.IsoBuffer.from(testArrayUtf8[index]);
			assert.equal(nodeBuffer1.toString("base64"), testArrayBase64[index]);

			const nodeBuffer2 = BufferNode.IsoBuffer.from(nodeBuffer1.toString("base64"), "base64");
			assert.equal(nodeBuffer2.toString(), testArrayUtf8[index]);

			const browserBuffer1 = BufferBrowser.IsoBuffer.from(testArrayUtf8[index]);
			assert.equal(browserBuffer1.toString("base64"), testArrayBase64[index]);

			const browserBuffer2 = BufferBrowser.IsoBuffer.from(
				browserBuffer1.toString("base64"),
				"base64",
			);
			assert.equal(browserBuffer2.toString(), testArrayUtf8[index]);
		}
	});
	...
	it("bufferToString working with IsoBuffer", () => {
		...
		assert.equal(BufferBrowser.bufferToString(buffer, "base64"), test);
		assert.equal(BufferBrowser.bufferToString(buffer, "utf-8"), "hellothere");

		const buffer2 = BufferNode.IsoBuffer.from(test, "base64");
		assert.equal(BufferNode.bufferToString(buffer2, "base64"), test);
		assert.equal(BufferNode.bufferToString(buffer2, "utf-8"), "hellothere");
	});
});
