	...
	policy: {
		...
		handlerExclusions: {
			...
			"npm-package-api-reports-match-entrypoints": [
				// Existing rollout debt. Keep entries package-specific so new packages are enforced.
				// Remove each entry when the package adds reportable declaration metadata and API reports,
				// or is marked private when it is not intended for publication.
				"^azure/packages/azure-local-service/package.json$",
				"^build-tools/packages/build-cli/package.json$",
				"^build-tools/packages/build-infrastructure/package.json$",
				// Synthetic package manifests used by build-infrastructure tests are not repository packages.
				...
				"^server/routerlicious/packages/services-ordering-zookeeper/package.json$",
				"^server/routerlicious/packages/services-shared/package.json$",
				"^server/routerlicious/packages/services-telemetry/package.json$",
